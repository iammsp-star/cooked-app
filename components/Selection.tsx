...
import { Music, Crosshair, ArrowRight, ArrowLeft } from "lucide-react";
import { useState } from "react";

interface SelectionProps {
  onSelect: (platform: "spotify" | "valorant", accountId: string) => void;
}

export default function Selection({ onSelect }: SelectionProps) {
  const [selectedPlatform, setSelectedPlatform] = useState<"spotify" | "valorant" | null>(null);
  ...
  return (
    <motion.div 
      className="flex flex-col items-center justify-center min-h-[80vh] gap-12 w-full max-w-4xl mx-auto"
          ...
          {!selectedPlatform ? (
            ...
            >
              {selectedPlatform === "spotify" ? (
                <Music className="w-16 h-16 text-[#ff4d4d]" />
              ) : (
                <Crosshair className="w-16 h-16 text-[#ff4d4d]" />
              )}
              
              <div className="text-center w-full">
                <h3 className="text-2xl font-bold font-mono uppercase text-[#ff4d4d] mb-2">
                  {selectedPlatform === "spotify" ? "Enter Spotify ID" : "Enter Riot ID"}
                </h3>
                <p className="text-zinc-400 text-sm mb-6">
                  {selectedPlatform === "spotify" ? "e.g., your username or profile link" : "e.g., TenZ#NA1"}
                </p>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full">
                  <input
                    type="text"
                    required
                    placeholder={selectedPlatform === "spotify" ? "Spotify Username" : "Riot ID #TAG"}
                    value={accountId}
